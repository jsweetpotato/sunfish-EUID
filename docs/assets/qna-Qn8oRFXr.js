import"./tailwind-hLRvZ190.js";import{S as x}from"./swiper-core-xIVJDqhu.js";import{g as h}from"./index-LZz_RBWS.js";import{a as w,g as b}from"./getNode-pa7syr6m.js";import{i as o}from"./insert-c9FKGVoP.js";import{c as i}from"./clear-vWyKvuJ4.js";import{p as d}from"./delay-L14dHocY.js";import{c as v}from"./checkAuth-CTdHH8Oq.js";import"./Modal-LwrVZ11o.js";new x(".swiper",{slidesPerView:"auto",spaceBetween:12,freeMode:!0});const g={interestsState:"all"};let l="-created";function y(){return`<li>
...
</li>`.repeat(7)}function S(e){const{id:t,category:a,title:s,imgField:f,views:p,created:u,comments:m}=e;let{description:r}=e;r=r.replace(/</g,"&lt;").replace(/>/g,"&gt;");const n=d.files.getUrl(e,f[0],{thumb:"0x60"});return`
  ...
  `}function $(e){const t=[];return e.forEach(a=>{t.push(S(a))}),t}function k(){i("#board-list"),o("#board-list",`
   <div class="sorry p-3 flex flex-col text-center">
     <span class="text-heading-2xl">😅</span>
     <p class="p-1 text-paragraph-lg">게시물이 없습니다.</p>
   </div>
   `),h.from(".sorry",{y:30,opacity:0,duration:.2})}function j(e){if(e.length<1){k();return}i("#board-list"),o("#board-list",e.join(""))}function C(e){const t=[];return e.interestsState!=="all"&&t.push(`category = "${e.interestsState}"`),t.join("&&")}async function c(){if(!v())return;i("#board-list"),o("#board-list",y());const e=C(g);try{const t=await d.collection("qAndA").getFullList({sort:l,filter:e});j($(t))}catch{alert("서버 통신을 하는 도중 오류가 발생했습니다. 잠시후 다시 시도해주세요.")}}c();const L=w(".category-button");function T({target:e}){const{id:t}=e;g.interestsState=t,c()}L.forEach(e=>{e.addEventListener("change",T)});const q=b("#sort");function A(e=1e3){let t=!0,a=!1;return s=>{a?alert("성격이 급하신 것 같아요. 천천히 눌러주세요~"):(t?(s.currentTarget.textContent="오래된 작성 순",s.currentTarget.classList.replace("bg-direction-icon","bg-direction_rotate-icon"),l="+created"):(s.currentTarget.textContent="최근 작성 순",s.currentTarget.classList.replace("bg-direction_rotate-icon","bg-direction-icon"),l="-created"),t=!t,c()),a=!0,setTimeout(()=>{a=!1},e)}}q.addEventListener("click",A());
