import"./tailwind-0m-bHF3b.js";import{S as y}from"./swiper-core-xIVJDqhu.js";import{g as v}from"./index-LZz_RBWS.js";import{a as u,g as S}from"./getNode-pa7syr6m.js";import{c as g,i as d}from"./clear-lK4FtH1n.js";import{p as h}from"./delay-L14dHocY.js";import{c as $}from"./checkAuth-CTdHH8Oq.js";import"./Modal-LwrVZ11o.js";new y(".swiper",{slidesPerView:"auto",spaceBetween:12,freeMode:!0});const f={interestsState:"all",filter:"filterAll"};let p="-created";function C(){return`<li>
  <div
  ...
  >
    ...
    `}function L(e){const t=[];return e.forEach(a=>{t.push(k(a))}),t}function T(){g("#board-list"),d("#board-list",`
   <div class="sorry p-3 flex flex-col text-center">
     <span class="text-heading-2xl">😅</span>
     <p class="p-1 text-paragraph-lg">게시물이 없습니다.</p>
   </div>
   `),v.from(".sorry",{y:30,opacity:0,duration:.2})}function j(e){if(e.length<1){T();return}g("#board-list"),d("#board-list",e.join(""))}function D(e){const t=[],a={all:"",project:"프로젝트",study:"스터디",food:"음식",hobby:"취미/여가",sports:"운동",reading:"독서",filterAll:"",filterOpen:"true",filterJoin:`members ~ "${h.authStore.model.id}"`};return e.interestsState!=="all"&&t.push(`category = "${a[e.interestsState]}"`),e.filter==="filterOpen"&&t.push(`isOpen = ${a[e.filter]}`),e.filter==="filterJoin"&&t.push(a[e.filter]),t.join("&&")}async function o(){if(!$())return;g("#board-list"),d("#board-list",C());const e=D(f);try{const t=await h.collection("together").getFullList({filter:e,sort:p});j(L(t))}catch{alert("서버 통신을 하는 도중 오류가 발생했습니다. 잠시후 다시 시도해주세요.")}}o();const E=u(".category-button");function A({target:e}){const{id:t}=e;f.interestsState=t,o()}E.forEach(e=>{e.addEventListener("change",A)});const B=S("#sortCreated");function O(e=1e3){let t=!0,a=!1;return r=>{a?alert("성격이 급하신 것 같아요. 천천히 눌러주세요~"):(t?(r.currentTarget.textContent="오래된 작성 순",r.currentTarget.classList.replace("bg-direction-icon","bg-direction_rotate-icon"),p="+created"):(r.currentTarget.textContent="최근 작성 순",r.currentTarget.classList.replace("bg-direction_rotate-icon","bg-direction-icon"),p="-created"),t=!t,o(),a=!0,setTimeout(()=>{a=!1},e))}}B.addEventListener("click",O());const F=u('input[name="filter"]');function N({target:e}){const{id:t}=e;f.filter=t,o()}F.forEach(e=>{e.addEventListener("change",N)});
